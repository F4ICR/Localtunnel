...
class TunnelDurationLogger:
    ...
    def check_previous_session(self):
        ...
        try:
            if os.path.exists("/tmp/tunnel_start_time.txt"):
                ...
                # Récupérer l'URL depuis le fichier dédié si disponible
                url = "URL inconnue"
                if os.path.exists("/tmp/last_tunnel_url.txt"):
                    try:
                        with open("/tmp/last_tunnel_url.txt", "r") as f_url:
                            url = f_url.read().strip()
                        logger.info(f"Récupération de l'URL depuis last_tunnel_url.txt : {url}")
                    except Exception as e:
                        logger.error(f"Erreur de lecture de last_tunnel_url.txt : {e}")
                # Sinon, tenter de récupérer depuis le backup si présent
                elif os.path.exists("/tmp/tunnel_backup.txt"):
    ...
    def clean_temp_files(self):
        """
        Supprime tous les fichiers temporaires utilisés pour le stockage
        """
        try:
            for file in ["/tmp/tunnel_start_time.txt", "/tmp/last_tunnel_url.txt", "/tmp/tunnel_backup.txt"]:
    ...
    def start_tunnel(self, url: str):
        ...
        # Sauvegarder l'URL dans un fichier dédié dès le début du tunnel
        try:
            with open("/tmp/last_tunnel_url.txt", "w") as f:
                f.write(url)
            logger.info(f"URL sauvegardée dans /tmp/last_tunnel_url.txt : {url}")
        except Exception as e:
            logger.error(f"Erreur lors de la sauvegarde de l'URL dans /tmp/last_tunnel_url.txt : {e}")
