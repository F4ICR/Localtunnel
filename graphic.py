...
def generer_graphique_cumule(donnees_par_date, output_file, jours_glissants, duree_minimale):
    ...
    # Ajout des informations du filtre et des jours glissants dans le titre
    plt.title(
...
def main():
    while True:
        ...
        if choix == "1":
            ...
            saisie_utilisateur = input("Entrez une date au format YYYY-MM-DD (ou appuyez sur Entrée pour aujourd'hui) : ")
            date_cible = saisie_utilisateur.strip() if saisie_utilisateur.strip() else str(date.today())

            try:
                datetime.strptime(date_cible, "%Y-%m-%d")  # Validation du format de la date
            except ValueError:
